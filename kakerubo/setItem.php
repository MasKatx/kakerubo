...
<head>
	...
	<title>家計簿管理サイト|kakerubo-費目の設定</title>
				...
				<section>
					...
					<form method="POST" action="<?php echo es($_SERVER['PHP_SELF']); ?>" style="text-align:center;">
						...
						// 費目設定
							try{
                                ...
                                if(isSet($_POST["name"])){
                                    $itemName = $_POST["name"];
                                    $itemMemo = $_POST["memo"];
                                    $sql = "select name from items where name = '$itemName';";
                                    $stm = $pdo->prepare($sql);
                                    $stm->execute();
                                    $result = $stm->fetchAll(PDO::FETCH_ASSOC);
									// 同一費目名がないかどうかの確認(unique)
                                    if($result != ""){
                                        if($itemMemo = ""){
											// メモがない場合はnullで入力
											...
											// メモがある場合はどちらも入力
                                            $sql = "insert into items(name, memo) values ('$itemName', ?);";
                                            $stm = $pdo->prepare($sql);
                                            $stm->bindValue(1, $itemMemo);
                                            $stm->execute();
                                        }
                                        header("Location:setItem.php");
                                    } else {
										// 同じ費目名の場合のエラー
                                        echo "<p class='c' style='color:#FF5555;'>費目が重複しています</p>";
                                    }
                                }
                            } catch (Exception $e){
                                echo $e->getMessage();
                        ...
                        try{
                            ...
                            $pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
                            $pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, PDO::ERRMODE_EXCEPTION);
                            $sql = "select name, memo from items;";
                            $stm = $pdo->prepare($sql);
                            $stm->execute();
                            $result = $stm->fetchAll(PDO::FETCH_ASSOC);
                        }catch (Exception $e){
                        ...
                        if(count($result) == 0){
							// 初回設定時の場合
                            echo "<p class='c'>費目がまだ設定されていません</p>";
                        } else {
                            echo"<table class='ta1'><tr><th>費目名</th><th>メモ</th></tr>";
							...
							echo "<table class='ta1'><tr><th>費目名</th>";
						...
						?>
						<p class="c">
							<input type="submit" value="費目を追加する" class="btn">
						</p>
					</form>
